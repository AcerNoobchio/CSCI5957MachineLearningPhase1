...
def plotAxis(xAxis, yAxis, filename):
    ...
    for i in range(1,yAxis.shape[0]):
        plt.plot(xAxis, yAxis[i], label='id %s' %i)
    plt.title(fileNameIn+" Limit: 500")
    plt.xlabel('Time')
    plt.ylabel('Feature')
    plt.legend()
...
def LoadDataByColumn(directory, columnsToRead, xAxis):
    data = []
    for col in columnsToRead:
        data.append(ReadColumnByFile(directory, col))
        plotAxis(xAxis, ReadColumnByFile(directory, col),"feature")
